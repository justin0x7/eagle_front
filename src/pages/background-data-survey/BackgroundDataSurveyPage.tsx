...
export default function BackgroundDataSurveyPage() {
  ...
  const [country, setCountry] = useState<null | string>(backgroundData?.country || "Sweden");
  ...
  useEffect(() => {
    if (!loadingBackgroundSurveyBasicData && !!backgroundSurveyBasicData) {
      ...
      Object.keys(backgroundSurveyBasicData).forEach((entityPluralName) => {
        ...
        const basicDataUnits = backgroundSurveyBasicData[entityPluralName as keyof BackgroundSurveyBasicData];
        basicDataUnits.forEach(basicDataUnit => {
          const nId = basicDataUnit.id;
          const textValue = backgroundData?.formDataByEntityName[entityName].filter(val => typeof val === "string").at(0) as string || "";
          const booleanValue = backgroundData?.formDataByEntityName[entityName].includes(nId) || false;
  ...
  }, [loadingBackgroundSurveyBasicData, backgroundSurveyBasicData, backgroundData, codeNumber, isFetched]);
  ...
  const handleSave = async () => {
    if (formData) {
      try {
        ...
        newBackgroundData.date = strDate;
        newBackgroundData.yearOfBirth = yearOfBirth || dayjs().get("year");
  ...
  return (
    <Container maxWidth="lg">
      <Typography variant="h3" align="center">{t("SurveyTitle", { ns: "BackgroundSurvey" })}</Typography>

      <Divider variant="middle" sx={{ my: 5 }} />

      <Grid container spacing={2}>
        <Grid item xs={6}>
          {t("SubTitle", { ns: "BackgroundSurvey" })}
        </Grid>
        <Grid item xs={6}>
          <table>
            <tbody>
              ...
              <tr>
                ...
                <td>
                  <TextField
                    ...
                    size="small"
                    type="number"
                    value={backgroundData?.yearOfBirth ? backgroundData?.yearOfBirth : yearOfBirth}
                    onChange={e => setYearOfBirth(backgroundData?.yearOfBirth ? backgroundData?.yearOfBirth : +e.target.value)}
                  />
                </td>
              </tr>
              <tr>
                ...
                <td>
                  <Autocomplete
                    ...
                    id="countries"
                    size="small"
                    value={backgroundData?.country ? backgroundData?.country : country}
                    onChange={(_e, newVal) => setCountry(backgroundData?.country ? backgroundData?.country : newVal)}
      ...
      {!!formData && !!backgroundSurveyFormMetadata && (
        <Stack gap={3}>
          {backgroundSurveyFormMetadata.map((metaData, metaDataIndex) => {
            return (
              <FormControl component="fieldset" fullWidth key={metaDataIndex}>
                <FormLabel component="legend">
                  <Typography variant="h6" color="#33A474">{t(metaData.label, { ns: "BackgroundSurvey" })}</Typography>
                </FormLabel>
                ...
                {singleChoiceEntityNames.includes(metaData.entityName) ? (
                  ...
                  >
                    {metaData.entitiesData.map((basicDataUnit, bIndex) => (
                      <FormControlLabel key={bIndex} value={basicDataUnit.id} control={<Radio />} label={t(basicDataUnit.description, { ns: "BackgroundSurvey" })} />
                    ))}
                  </RadioGroup>
                ) : (
                  <>
                        ...
                        basicDataUnit.description !== "OtherText" && (
                          <FormControlLabel
                            ...
                            control={
                              ...
                              />}
                            label={t(basicDataUnit.description, { ns: "BackgroundSurvey" })}
                          />
                        ))
                      )}
